...
export default function Dashboard({ auth }) {
    ...
    const pasPhoto = auth.user?.registration?.documents?.find(
        doc => doc.document_type === 'pas photo' || doc.document_type === 'pas_foto'
    );
                        ...
                        <div className="mt-8 bg-gradient-to-r border-2 from-secondary/10 p-6 rounded-lg shadow-sm">
                            ...
                            <div className="space-y-2">
                                ...
                                <p className="text-gray-700">
                                    ...
                                    {auth.user?.registration?.status !== 'menunggu' && auth.user?.registration?.created_at &&
                            ...
                            {auth.user?.registration ? (
                                <ul className="space-y-4">
                                    {auth.user.registration.documents?.length > 0 ? (
                                        auth.user.registration.documents.map((doc, index) => (
                                            <li key={index} className="flex items-center justify-between bg-white p-4 rounded-lg shadow-sm">
                                                <div className="flex-1">
                                                    ...
                                                    <span className={`px-2 ml-1 py-1 rounded text-white text-sm font-semibold ${getVerificationStatusColor(doc.verification?.status)}`}>
                                        ...
                                        <p className="text-gray-600 text-center">Belum ada dokumen yang diunggah.</p>
                                    )}
                                </ul>
                            ) : (
                                <div className="text-center">
                                    <p className="text-gray-600">Anda belum melakukan registrasi, silakan registrasi terlebih dahulu.</p>
